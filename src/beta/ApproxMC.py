...
def bwsat(hashClauses, pivot, r, wmax,t): 
    ...
    # 2. recover mapping from cnf to smt variables
    ...
    # 3. read and add hash clauses (augmented dimacs format http://www.msoos.org/xor-clauses/)
    inverted = False # i.e., when no '-'' exists, add Clause == !inverted
    for line in lines: # a clause  
            Clause = False 
            for keys in line.strip('x').strip('\n').split(' '):
                if not keys.isdigit(): 
                    continue
                ...
                if (key != 0): # dimacs EOF
                    if varmap[abs(key)+offset] == None: # skip tseitin variables
                        continue
                    if key > 0:
                        Clause = Xor(Clause,varmap[key+offset])
                    else: 
                        inverted = Not(inverted) # allow for two or more '-' 
                        Clause = Xor(Clause,varmap[abs(key)+offset])

            # add Clause 
            s.add(Clause == Not(inverted)) 

    # 4. do WMI computation on s 
    count, wmi, wmin = bounded_wmi(s, t.pwd, t.ld, t.bounds, t.dvars, pivot, r, wmax, wmin)

    # line 11 of algo 3    
    return count, wmi, wmin*r
