# Code by Vaishak Belle for ApproxWMI, and builds on http://arxiv.org/pdf/1404.2984, Distribution-Aware Sampling and Weighted Model Counting for SAT. Supratik Chakraborty, Daniel J. Fremont, Kuldeep S. Meel, Sanjit A. Seshia, and Moshe Y. Vardi. Proceedings of AAAI Conference on Artificial Intelligence (AAAI), 2014.  
...
def bwsat(lines, pivot, r, wmax,t): 
    # nos models and wmi
    valCount = 0 # @vmay non-wmi case valCount = len(lines)
    wmi = 0 
...
# @vmay, maxsol plus 1
    if (noSolutions):
        ...
        cmd = "./doalarm -t profile "+str(timeout)+" ./cryptominisat --gaussuntil=400 --maxsolutions="+str(pivot+1)+" --verbosity=0 "+str(fileName)+"| grep \"v \""+" > "+str(outputFileName) 
    else:
        cmd = "./doalarm -t profile "+str(timeout)+" ./cryptominisat --gaussuntil=400 --maxsolutions="+str(pivot+1)+" --verbosity=0 "+str(fileName)+" > "+str(outputFileName)
    starttime = time.time()
    #print cmd
    os.system(cmd)
    endtime = time.time()
    ...
    if (not(noSolutions)): # @vmay,  and valCount!=0
...
def ApproxWMI(runIndex,timeout,initialFileName,numVariables,numClauses,pivot,numIterations, tilt, shouldLog,logFileName,finalFileName,initialFileNameSuffix, t):
    ...
    for i in range(numIterations):
        ...
        # @vmay instead of int(wmi/wmax) <= maxSolutions
        while(hashCount < numVariables):  
            ...
            if (isSolvable == 0):
                ...
                wmaxSolList.append(wmax) # @vmay totalSolutions*wmax
                break
